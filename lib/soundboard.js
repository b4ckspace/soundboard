...
module.exports = {
	getSoundData() {
		...
		return keys
			.filter(key => fs.statSync(soundDir + '/' + key).isDirectory())
			.map((key) => {
				const sounds = fs
					.readdirSync(soundDir + '/' + key)
					.filter(filename => fs.statSync(soundDir + '/' + key  + '/' + filename).isFile())
					.map((filename) => {
						return {
							filename: filename,
	...
	play(groupName, soundName) {
		...
		let group = soundData.find(group => group.name === groupName);
		if (!group) {
